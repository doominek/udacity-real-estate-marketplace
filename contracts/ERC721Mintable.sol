...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused {
        require(_paused == false, "Only allowed when not paused");
        _;
    }

    modifier whenPaused {
        require(_paused == true, "Only allowed when paused");
        _;
    }
}
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public onlyForExistingToken(tokenId) {
        address owner = ownerOf(tokenId);
        require(owner != to, "Token is already owned by receiver");
        require(owner == msg.sender || isApprovedForAll(owner, msg.sender), "Sender must be owner or approved operator");
    ...
    modifier onlyForExistingToken(uint256 tokenId) {
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function tokenURI(uint256 tokenId) external onlyForExistingToken(tokenId) view returns (string memory) {
        return _tokenURIs[tokenId];
    }

    function _setTokenURI(uint256 tokenId) internal onlyForExistingToken(tokenId) {
        _tokenURIs[tokenId] = strConcat(_baseTokenURI, uint2str(tokenId));
    }

}

contract DREMToken is ERC721Metadata {
    constructor () public
    ERC721Metadata("Dom's Real Estate Marketplace", "dREM", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
    }
